...
// Candid IDL factory for the OneBlock profile canister
export const idlFactory: IDL.InterfaceFactory = ({ IDL }) => {
  ...
  const Wallet = IDL.Record({
    name: IDL.Text,
    addresses: IDL.Vec(IDL.Record({ network: Network, address: IDL.Text })),
  });
...
export function createOneBlockActor() {
  const agent = new HttpAgent({ host: "https://ic0.app" });
  return Actor.createActor(idlFactory, {
    agent,
    canisterId: ONEBLOCK_CANISTER_ID,
  });
}
